...
def main(argc, argv):
    ...
    try:
        ...
        if argc < 2:
            raise CmdSyntaxError("0","Por favor informe o número da lista, o número da questão e parâmetros para continuar a execução.")
        ...
        if exec_list == "2":
            ...
            if question_number == "5":
                exit_code = mcmc_engines.l2_exec_quest5(argc,argv) 
            elif question_number == "7":
                exit_code = mcmc_engines.l2_exec_quest7(argc,argv) 
            elif question_number == "9":
                exit_code = mcmc_engines.l2_exec_quest9(argc,argv) 
            else:
            ...
            if question_number == "2":
                exit_code = mcmc_engines.l3_exec_quest2(exp_id,argc,argv) 
            elif question_number == "3":
                exit_code = mcmc_engines.l3_exec_quest3(exp_id,argc,argv) 
            elif question_number == "4":
                exit_code = mcmc_engines.l3_exec_quest4(exp_id,argc,argv) 
            else:
                raise CmdSyntaxError("L2Q0","Por favor informe 2, 3  para o número da questão.")
        else:
