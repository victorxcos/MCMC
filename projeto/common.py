...
L4_QUEST3_IN_LINE_CMD_SYNTAX=("python3 mcmc_exec.py 4 3 <parameters> [-plot]\n",
                    "\n",
                    "To execute a experiment for question 3 in list 4, use:\n",
                    "python3 mcmc_exec.py 4 3 <reject|MH> <num_vertex> [ <out_grade> ] [ <num_exec> ] [-plot]\n" + 
                    "\n",
                    "Examples:\n",
                    "python3 mcmc_exec.py 4 3 reject 100 5 10 \n",
                    )

SIM_ALL_IN_LINE_CMD_SYNTAX=("python3 mcmc_exec.py 0 <simulation_type> <parameters> [-plot]\n",
                    "\n",
                    "To execute a experiment, please inform the simulation type and the parameters, use:\n",
                    "python3 mcmc_exec.py 0 <simulation_type> <parameters> [-plot]\n" + 
                    "\n",
                    "Examples:\n",
                    "python3 mcmc_exec.py 0 5 uniform 100 \n",
                    "python3 mcmc_exec.py 0 7 10 \n"
                    )

SIM_CLUSTER_IN_LINE_CMD_SYNTAX=("python3 mcmc_exec.py 0 clustering <parameters> [-plot]\n",
                    "\n",
                    "To execute a experiment for clustering simulation, use:\n",
                    "python3 mcmc_exec.py 0 clustering <input_file> <delta|linear|log|exp|quadr|all> <uniqueness|random> [<num_steps>] [<num_exec>] [<beta_rate>] [<initial_config>] [-plot]\n" + 
                    "\n",
                    "Examples:\n",
                    "python3 mcmc_exec.py 0 clustering ./input/debug.csv linear uniqueness 10000 10 \n",
                    "python3 mcmc_exec.py 0 clustering ./input/debug.csv linear random 10000 10 0.5 \n",
                    "python3 mcmc_exec.py 0 clustering ./input/debug.csv exp uniqueness 1000 1 0.9 1000 1a -plot\n",
                    "python3 mcmc_exec.py 0 clustering ./input/debug.csv all uniqueness 1000 1 0.8 1000 1a;4a -plot\n"
                    )

IN_LINE_CMD_SYNTAX={ "ALL" : ALL_IN_LINE_CMD_SYNTAX,
                     "S000" : SIM_ALL_IN_LINE_CMD_SYNTAX, 
                     "S001" : SIM_CLUSTER_IN_LINE_CMD_SYNTAX, 
                     ...
                     "L3Q0" : L2_QUEST0_IN_LINE_CMD_SYNTAX,
                     "L3Q2" : L3_QUEST2_IN_LINE_CMD_SYNTAX,
                     "L3Q3" : L3_QUEST3_IN_LINE_CMD_SYNTAX,
                     "L3Q4" : L3_QUEST4_IN_LINE_CMD_SYNTAX,
                     "L4Q0" : L3_QUEST4_IN_LINE_CMD_SYNTAX,
                     "L4Q3" : L3_QUEST4_IN_LINE_CMD_SYNTAX
                    }

point_markers = ['bo','ro','yo','co','go','bs','rs','ys','cs','gs','bd','rd','yd','cd','gd' ]

DATASET_ELEMENT_SEPARATOR=","
...
def display_progress(milestone,pace=0):
    ...
    if (pace == 0) or ( (pace * 100) % 10 == 0 ) :        
        print(milestone, end=end_line, flush=True)
